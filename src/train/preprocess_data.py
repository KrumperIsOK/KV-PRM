...
def _ppm_pairs_from_tree(
    ...
    *,
    ...
    Returns preference-pair records.
    """
...
def main():
    ...
    ap.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory or JSON file containing MCTS trees",
    )
    ap.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Output PRM JSONL (default: next to --input)",
    )
    ap.add_argument(
        "--target-nodes",
        type=int,
        default=5,
        help="Exact path length in nodes (root+steps)",
    )
    ap.add_argument(
        "--dedupe",
    ...
    if not args.input.exists():
        ap.error(f"Input path does not exist: {args.input}")
    prm_output_path = args.output or args.input.with_name(
        f"prm_samples_{args.input.stem}.jsonl"
    )
    prm_output_path.parent.mkdir(parents=True, exist_ok=True)

    ppm_output_path = (
        args.input.with_name(f"ppm_pairs_{args.input.stem}.jsonl")
        if args.make_ppm
        else None
    )
    ...
    json_files = (
        [args.input] if args.input.is_file() else sorted(args.input.rglob("*.json"))
    )

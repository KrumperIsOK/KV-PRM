...
def is_correct(pred, truth) -> bool:
    """
    Unified answer checker used across rollout and evaluation scripts.
...
def extract_answer_math(output):
    """
    Extract answer from model prediction for MATH dataset.
    """
    output_str = last_boxed_only_string(output)
    if output_str:
        output_rmb = remove_boxed(output_str)
    else:
        if "the final answer is" in output.lower():
            output_rmb = (
                output.lower()
                .split("the final answer is")[-1]
                .strip()
                .split(".")[0]
                .strip()
            )
            ...
            output_rmb = (
                output.lower().split("final answer")[-1].strip().split(".")[0].strip()
            )
        elif "answer" in output.lower():
            output_rmb = (
                output.lower().split("answer")[-1].strip().split(".")[0].strip()
            )
        elif "final refined solution" in output.lower():
            output_rmb = (
                output.lower()
                .split("final refined solution")[-1]
                .strip()
                .split(".")[0]
                .strip()
            )
            ...
            output_rmb = (
                output.lower()
                .split("refined solution")[-1]
                .strip()
                .split(".")[0]
                .strip()
            )
        else:
            output_rmb = output.strip().split(".")[-1].strip()

    if output_rmb is not None:
        output_striped = _strip_string(output_rmb)
    else:
        output_striped = ""
    return output_striped


def is_correct_math(pred, truth, verbose=False):
    """
    Check if model prediction is equivalent to ground truth for MATH dataset.
    """
    pred_answer = extract_answer_math(pred)
    # truth_answer = extract_answer_math(truth)
    return is_equiv(pred_answer, truth, verbose=verbose)

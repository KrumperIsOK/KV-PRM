...
class MAS:
    ...
    def step(self, messages: List[str], idx: int, **gen_kwargs) -> List[str]:
        sys_prompt = getattr(
            self.agents[idx], "system_prompt", "You are a helpful assistant."
        )
        content = "\n\n".join(messages).strip()
        msgs = [
    ...
    # The second return value records each agent's inputs and outputs.
    def generate(
        ...
        frontier = {
            j for j in frontier if required_parents[j].issubset(set(inbox[j].keys()))
        }

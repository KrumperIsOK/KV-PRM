...
def _deliver_outputs(
    mas: MAS, agent_idx: int, outs: List[str], inbox: Dict[int, Dict[int, str]]
):
    ...
    def _rollout(self, root: Node) -> List[Node]:
        ...
        while True:
            ...
            if not node.children:
                ...
                replay_state, _, _ = _replay_trajectory(
                    self.mas, self.question, node.trajectory
                )
                inbox = replay_state
                ...
                for outs in candidates:
                    ...
                    if self._depth_of(child) >= self.mas.n:
                        ...
                        child.final_answer = _aggregate_final(
                            self.mas, primary_out2, last2
                        )

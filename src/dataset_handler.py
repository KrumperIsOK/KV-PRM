...
EVAL_ONLY_DATASETS = {"aime_2024", "aime_2025"}
REPO_ROOT = Path(__file__).resolve().parents[1]


def load_hard_dataset(name, split, n, seed):
    if name in EVAL_ONLY_DATASETS and split == "train":
        raise ValueError(
            f"Dataset '{name}' is evaluation-only and has no train split. "
            f"Use split='test' for evaluation."
        )

    if name == "competition_math":
        dataset_path = REPO_ROOT / "data" / "math.parquet"
        if not dataset_path.exists():
            raise FileNotFoundError(f"Bundled MATH dataset not found: {dataset_path}")
        ...
        data = ds["train"]  # single HF split contains all 30 problems
        if n:
            data = data.shuffle(seed=seed).select(range(min(n, len(data))))
        gold_fn = lambda ex: _to_number(str(ex["answer"]))
        q_fn = lambda ex: ex["problem"]
        ...
        raise ValueError(
            "Unsupported dataset. Choose from: competition_math, aqua_rat, svamp, "
            "gsm8k, mmlu, gpqa, logiqa, aime_2024, aime_2025"
        )

...
def compute_logits_metrics(
    ...
    *,
    ...
    """
    Computes log probabilities and derived metrics for a given completion.

    Returns a dictionary containing:
      - 'sum_logprob': Total log probability (negative, large magnitude).
      - 'avg_logprob': Log probability normalized by token length.
      - 'perplexity':  Exponential of negative avg_logprob (lower is better).
      - 'prompt_len':  Number of tokens in the prompt.
      - 'gen_len':     Number of tokens in the completion.
      - 'total_len':   Total tokens (prompt + completion).
    """
    tok = agent.tok
    model = agent.model
    device = next(model.parameters()).device
    ...
    if prompt_ids is None:
        ...
        if isinstance(enc, list):
            ...
            # Handle cases where chat template returns string (e.g. older tok)
            s = tok.apply_chat_template(
                msgs,
                tokenize=False,
                add_generation_prompt=True,
        ...
        # User provided pre-tokenized prompt
        if isinstance(prompt_ids, list):
            prompt_ids = (
                torch.tensor(prompt_ids, dtype=torch.long).unsqueeze(0).to(device)
            )
    ...
    # --- 2. Prepare Completion IDs ---
    comp_ids = tok(completion, return_tensors="pt", add_special_tokens=False)[
        "input_ids"
    ].to(device)

    # Handle empty completion edge case
    if comp_ids.shape[1] == 0:
        return {
            ...
            "prompt_len": int(prompt_ids.shape[1]),
            "gen_len": 0,
            "total_len": int(prompt_ids.shape[1]),
        }

    # --- 3. Forward Pass ---
    input_ids = torch.cat([prompt_ids, comp_ids], dim=1)

    # Simple attention mask (assuming no padding needed for batch size 1)
    attn = torch.ones_like(input_ids, device=device)

    out = model(input_ids=input_ids, attention_mask=attn)

    # Logits match input_ids shifted by one
    logits = out.logits[:, :-1, :]  # [1, L-1, V]
    target = input_ids[:, 1:]  # [1, L-1]

    # --- 4. Align Prompt/Completion Indices ---
    prompt_len = int(prompt_ids.shape[1])
    gen_len = int(comp_ids.shape[1])

    # The logit at index `prompt_len - 1` predicts the first completion token
    start = max(0, prompt_len - 1)
    end = start + gen_len

    # --- 5. Vectorized Calculation ---
    # LogSoftmax only on the relevant slice to save compute
    lp_slice = F.log_softmax(logits[0, start:end, :], dim=-1)  # [gen_len, V]
    tgt_slice = target[0, start:end].unsqueeze(-1)  # [gen_len, 1]

    # Gather the logprobs for the actual tokens that appeared in `completion`
    token_logprobs = lp_slice.gather(-1, tgt_slice).squeeze(-1)  # [gen_len]

    sum_logprob = token_logprobs.sum().item()
    avg_logprob = sum_logprob / gen_len
...
def score_path_logprob(
    ...
    *,
    ...
    for j, text in enumerate(steps):
        ...
        sys_prompt = getattr(
            mas.agents[j], "system_prompt", "You are a helpful assistant."
        )
        msgs = [
...
def sbs_decode2(
    mas: Any,
    question: str,
    score_type: str = "none",  # "none" | "kvprm" | "prm" | "logprob"
    ...
    return_tree: bool = False,  # NEW PARAMETER
    *,
    ...
    # --- TREE INIT ---
    # We initialize a root node for the tree
    root_node = {
        "id": str(uuid.uuid4()),
        "type": "root",
        "agent_idx": -1,
    ...
    # beams now stores: (trajectory_dict, score, parent_node_reference)
    beams: List[Tuple[Dict[int, List[str]], float, Dict[str, Any]]] = [
        ({}, 0.0, root_node)
    ]
    ...
    for agent_idx in range(mas.n):
        # Temp list to hold (new_traj, new_score, new_node)
        all_candidates_for_step: List[
            Tuple[Dict[int, List[str]], float, Dict[str, Any]]
        ] = []

        for traj, _, parent_node in beams:
            state, _, _ = _replay_trajectory(mas, question, traj)
            ...
            for outs in cands:
                if outs:
                    usage.generated += _text_token_len_agent(
                        mas.agents[agent_idx], outs[0]
                    )
            ...
            for outs in scored_cands:
                if score_type == "prm":
                    ...
                    # Placeholder for KV-PRM logic
                    traj2 = {**traj, agent_idx: outs}
                    new_msgs = msgs + [{"role": "assistant", "content": outs[0]}]
                    s_text = mas.agents[agent_idx].tok.apply_chat_template(
            ...
            for outs, v in cand_infos:
                text_content = outs[0] if outs else ""
                ...
                new_node = {
                    ...
                    "children": [],
                    "selected": False,  # Will determine later if this survives
                    "metadata": {"full_outputs": outs, "score_type": score_type},
                }

                # Attach to parent in the tree
                parent_node["children"].append(new_node)

                # Add to candidate list for sorting
                new_traj = {**traj, agent_idx: outs}
                all_candidates_for_step.append((new_traj, v, new_node))

            # Legacy trace logic (optional)
        ...
        # Sort all candidates from all beams for this step
        if score_type != "none":
            all_candidates_for_step.sort(key=lambda x: x[1], reverse=True)

        # Pruning: Select top B1
        survivors = all_candidates_for_step[: max(1, B1)]

        # Mark survivors in the tree and update beams list
        beams = []
        for traj, score, node in survivors:
            node["selected"] = True
            beams.append((traj, score, node))

        # Update legacy trace info
        if local_trace is not None and len(local_trace) > 0:
            chosen_traj, chosen_val, _ = beams[0]
    ...
    ret = [final_answer, usage]
    if return_trace:
        ret.append(local_trace or [])
    if return_tree:
...
def load_prm_scorer(
    ...
    if base_model_id is None:
        ...
        base_model_id = getattr(cfg, "base_model_name_or_path", None) or "Qwen/Qwen3-4B"
...
def _resolve_num_verify_tokens(prm_dir: str, override: Optional[int]) -> int:
    """
    Resolve K (verify-token count at inference). Priority:
      1. explicit `override` arg
      2. `num_verify_tokens` field in <prm_dir>/prm_config.json
      3. fallback to 1 (legacy single-token checkpoints)
    """
    if override is not None:
        if override < 1:
            raise ValueError("num_verify_tokens must be >= 1")
        return int(override)
    cfg_path = Path(prm_dir) / "prm_config.json"
    if cfg_path.exists():
        try:
            ...
            with open(cfg_path, "r", encoding="utf-8") as f:
                cfg = _json.load(f)
            n = cfg.get("num_verify_tokens", 1)
            return max(1, int(n))
        except Exception as e:
            print(f"Warning: failed to read num_verify_tokens from {cfg_path}: {e}")
    return 1


def load_kv_prm_scorer(
    prm_dir: str,
    base_model_id: str = None,
    verify_token: str = "?",
    positive_token: str = "+",
    ...
    """
    Returns: (score: Callable[[str], float], tokenizer, model)
    Loads a CausalLM base + LoRA PRM adapter trained with KVTransferTrainer.
    Score = Probability of positive token (+) given verify token (<verify>) and history.
    Range: [0, 1] (representing mapped value of original labels [-1, 1])
    """
    # 1. Resolve base model if not supplied
    if base_model_id is None:
        try:
            cfg = PeftConfig.from_pretrained(prm_dir)
            ...
            raise ValueError(
                "Could not infer base_model_id from prm_dir, please provide it explicitly."
            )

    print(f"Loading kvprm base model: {base_model_id}")

    # 2. Tokenizer: prefer saved with adapter; else base
    tok_src = prm_dir if (Path(prm_dir) / "tokenizer.json").exists() else base_model_id
    ...
    # Resolve Token IDs
    # Note: Training script used add_special_tokens=False. We replicate that.
    def get_id(s):
        ids = tokenizer.encode(s, add_special_tokens=False)
        return ids[0] if ids else tokenizer.eos_token_id

    verify_id = get_id(verify_token)
    pos_id = get_id(positive_token)
    neg_id = get_id(negative_token)

    # Resolve K (number of verify tokens at inference) from prm_config.json if not given.
    K = _resolve_num_verify_tokens(prm_dir, num_verify_tokens)
    print(f"kv-PRM scorer using num_verify_tokens={K}")

    # 3. Base Model
    # Ignoring quantization as requested
    ...
    model = PeftModel.from_pretrained(base_model, prm_dir)
    model.eval()

    # Ensure model is on correct device (PeftModel usually handles this if base is on device, but to be safe)
    model.to(device)
    ...
    def score(text: str) -> float:
        ...
        # 2. Reference Pass (Get KV Cache) -- adapter disabled, matches training.
        ...
        # 3. Verify Pass -- K verify tokens, adapter active.
        batch_size = input_ids.shape[0]
        verify_input_ids = torch.full(
            (batch_size, K), verify_id, dtype=torch.long, device=device
        )
        verify_mask = torch.ones(
            (batch_size, K), dtype=attention_mask.dtype, device=device
        )
        extended_attention_mask = torch.cat([attention_mask, verify_mask], dim=1)
        ...
        # 4. Score from the LAST verify position.
        last_token_logits = outputs.logits[:, -1, :]
        target_logits = last_token_logits[:, [neg_id, pos_id]]
...
def load_text_prm_scorer(
    ...
    """
    Returns: (score: Callable[[str], float], tokenizer, model)
    Loads a CausalLM base + LoRA PRM adapter trained with FullSequencePRMTrainer.
    Single forward over [context | verify_token]; LoRA applies to all tokens.
    Score = P(+) in [0, 1].
    """
    if base_model_id is None:
        try:
    ...
    print(f"Loading text-PRM base model: {base_model_id}")
    ...
    K = _resolve_num_verify_tokens(prm_dir, num_verify_tokens)
    print(f"text-PRM scorer using num_verify_tokens={K}")
    ...
    def score(text: str) -> float:
        ...
        target_logits = last_logits[:, [neg_id, pos_id]]
        probs = torch.softmax(target_logits, dim=-1)
        return float(probs[0, 1].item())
...
def load_prm_scorer_auto(
    prm_dir: str,
    base_model_id: str = None,
    prm_encoding: str = None,
    **kwargs,
):
    """
    Auto-detect prm_encoding from prm_config.json in the checkpoint directory,
    then dispatch to load_kv_prm_scorer or load_text_prm_scorer.

    If prm_encoding is explicitly provided, it overrides auto-detection.
    Returns: (score_fn, tokenizer, model)
    """
    import json as _json

    if prm_encoding is None:
        config_path = Path(prm_dir) / "prm_config.json"
        if config_path.exists():
            with open(config_path, "r", encoding="utf-8") as f:
                cfg = _json.load(f)
            prm_encoding = cfg.get("prm_encoding", "kv")
            print(f"Auto-detected prm_encoding={prm_encoding!r} from {config_path}")
        else:
            prm_encoding = "kv"
            print(
                f"No prm_config.json found in {prm_dir}; defaulting to prm_encoding='kv'"
            )

    if prm_encoding == "text":
        return load_text_prm_scorer(prm_dir, base_model_id=base_model_id, **kwargs)
    else:
        return load_kv_prm_scorer(prm_dir, base_model_id=base_model_id, **kwargs)
...
class MCTSInfer:
    ...
    def __init__(
        ...
        *,
        ...
        self.parent_order = {
            i: sorted(self.required_parents[i]) for i in range(self.mas.n)
        }
        self.sys_prompts = [
    ...
    def _expand(self, node: _Node) -> None:
        ...
        state, _, _ = _replay_trajectory(self.mas, self.q, node.traj)
        inbox = state
        have = inbox.get(d, {})
        ...
        for outs in candidates:
            if outs:
                self.usage.generated += _text_token_len_agent(
                    self.mas.agents[d], outs[0]
                )
        ...
        for outs in candidates:
            ...
            if self.score_type == "prm":
                v = (
                    ...
                    else float(self.score_fn(self._state_text(traj2)))
                )

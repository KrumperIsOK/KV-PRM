...
def main():
    ...
    ap.add_argument("--mas", type=str, default="sequential")
    ...
    ap.add_argument(
        "--use_openai", action="store_true", help="Use OpenAI API for generation"
    )
    ap.add_argument(
        "--openai_base_url",
        type=str,
        default=os.environ.get("OPENAI_BASE_URL", "https://router.huggingface.co/v1"),
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--output_dir",
        default="artifacts/trees",
        help="Directory in which to save the run folder.",
    )
    ...
    if not cfg_path.exists():
        ap.error(f"Unknown MAS configuration {args.mas!r}: {cfg_path} does not exist")
    cfg = yaml.safe_load(cfg_path.read_text())
    agent_specs = cfg["agents"]
    edges = cfg["edges"]

    thinking_flag = "" if not args.no_think else "_nothink"
    model_tag = args.model_id.rstrip("/").split("/")[-1]
    run_name = (
        f"{args.dataset}_{args.split}_{model_tag}{thinking_flag}_mas_{args.mas}_"
        f"mcts_{args.n_rollouts}_seed{args.seed}"
    )
    result_path = str(Path(args.output_dir) / run_name)
    ...
    if args.ray:
        ...
        workers = [
            RayWorker.options(
                num_gpus=args.gpus_per_actor if not args.use_openai else 0
            ).remote(
                args.model_id,
                device_map="cuda",
    ...
    remaining_idxs = [
        i for i in range(len(data)) if not Path(f"{result_path}/{i}.json").exists()
    ]
    pbar_sp = tqdm(
    ...
    for i, ex in enumerate(data):
        ...
        os.makedirs(result_path, exist_ok=True)
        json_data = node_to_dict(root)
        pathlib.Path(f"{result_path}/{i}.json").write_text(
